...
|모듈명|설명|처리 기능|
|BaseHTTPServer|서버 클래스로 HTTPServer 클래스가 정의되어 있음.|HTTP프로토콜 처리|
||핸들러 BaseHTTPRequestHandler 클래스 정의 ||
|SimpleHTTPServer|HTTPServer를 임포트 하여 사용 |GET/HEAD 메소드 처리|
||핸들러 SimpleHTTPRequestHandler 클래스 정의 ||
|CGIHTTPServer|HTTPServer를 임포트 하여 사용 |POST와 CGI 처리 |
